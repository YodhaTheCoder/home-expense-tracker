...
function MoneySummary({ summary, summaryFilter, setSummaryFilter }) {
  ...
  const pendingAmount = Math.max(Math.abs(balance) - paidAmount, 0);

  const percentage =
    balance !== 0 ? Math.min(Math.round((paidAmount / Math.abs(balance)) * 100), 100) : 0;
  ...
  return (
    <div className="money-summary-card">
      <div className={`money-summary-top ${status}`}>
        ...
        <h1 className={balance >= 0 ? 'balance-positive' : 'balance-negative'}>
          {summary.balance >= 0
      ...
      <div className="money-summary-content">
        <div className="money-summary-grid">
          <div className="money-box money-given">
            ...
            <strong>₹{Number(summary.given).toLocaleString()}</strong>
          </div>

          <div className="money-box money-taken">
            <span>Taken</span>

            <strong>₹{Number(summary.taken).toLocaleString()}</strong>
          </div>
        </div>
