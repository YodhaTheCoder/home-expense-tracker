  ...
  return (
    <div className="card expense-list-card">
      <div className="section-header">
        ...
        <span className="category-count">{moneyDues.length} Entries</span>
      </div>

      {moneyDues.length === 0 ? (
        <p className="muted">No money records found.</p>
      ) : (
        <div className="category-grid">
          {moneyDues.map((item) => {
            ...
            const paidAmount =
              item.money_payments?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;
              ...
              >
                <div className="category-top">
                  ...
                  <div className="category-details">
                    ...
                    <p className="username-text">{item.person_name}</p>

                    <p className="username-text">
                      {item.type === 'GIVEN' ? 'Money Given' : 'Money Taken'}
                    </p>
                ...
                {remaining === 0 ? (
                  <div className="category-type custom-type">Completed</div>
                ) : (
                  <div className="category-type custom-type">Pending</div>
                )}
