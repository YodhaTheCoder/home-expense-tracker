...
export function useMoneyTracker(
  ...
  const filteredMoneyDues = moneyDues.filter((item) => {
    // no filter selected
    if (!summaryFilter?.month && !summaryFilter?.year) {
      return true;
    }

    const date = new Date(item.transaction_date);

    const monthMatch = !summaryFilter.month || date.getMonth() + 1 === summaryFilter.month;

    const yearMatch = !summaryFilter.year || date.getFullYear() === summaryFilter.year;
  ...
  async function saveMoney(event, moneyForm, editingMoneyId) {
    ...
    try {
      ...
      if (editingMoneyId) {
        await updateMoneyDue(editingMoneyId, payload);

        setMessage('Money updated.');
      } else {
        await createMoneyDue(payload);
  ...
  function calculateSummary() {
    ...
    filteredMoneyDues.forEach((item) => {
      ...
      const paidAmount = (item.money_payments || []).reduce(
  ...
  async function editPayment(id, payload) {
    try {
